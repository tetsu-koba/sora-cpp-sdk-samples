...
struct SendOnlyConfig : sora::SoraDefaultClientConfig {
...
class SendOnly : public std::enable_shared_from_this<SendOnly>,
                  public sora::SoraDefaultClient {
 public:
  SendOnly(SendOnlyConfig config)
...
int main(int argc, char* argv[]) {
  ...
  SendOnlyConfig config;

  auto bool_map = std::vector<std::pair<std::string, bool>>(
      {{"false", false}, {"true", true}});

  CLI::App app("Send Only Sample for Sora C++ SDK");
  ...
  auto sendonly = sora::CreateSoraClient<SendOnly>(config);
  sendonly->Run();

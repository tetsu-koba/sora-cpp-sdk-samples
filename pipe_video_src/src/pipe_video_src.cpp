...
struct PipeVideoConfig : sora::SoraDefaultClientConfig {
...
class PipeVideo : public std::enable_shared_from_this<PipeVideo>,
                  public sora::SoraDefaultClient {
 public:
  PipeVideo(PipeVideoConfig config)
      : sora::SoraDefaultClient(config), config_(config) {}

  void Run() {
    if (!config_.no_video_device) {
      auto size = config_.GetSize();
      PipeVideoCapturerConfig pipe_config;
      pipe_config.width = size.width;
      pipe_config.height = size.height;
      pipe_config.framerate = config_.framerate;
      pipe_config.video_fifo = config_.video_fifo;
      auto video_source = PipeVideoCapturer::Create(pipe_config);
...
int main(int argc, char* argv[]) {
  ...
  PipeVideoConfig config;

  auto bool_map = std::vector<std::pair<std::string, bool>>(
      {{"false", false}, {"true", true}});

  CLI::App app("Pipe video Sample for Sora C++ SDK");
  ...
  app.add_option("--video-fifo", config.video_fifo,
                 "Fifo name for video. ('-' means stdin)")
    ->required();
  ...
  auto pipe_video = sora::CreateSoraClient<PipeVideo>(config);
  pipe_video->Run();
